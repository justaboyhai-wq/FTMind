...
// NewCmdReparse builds `fmind doc reparse <doc-id>`.
...
'fmind doc wait <doc-id>' to block until it reaches a terminal status.`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"fmind doc reparse doc_abc",
			"fmind doc reparse doc_abc --format json",
		},
		Output: "envelope.data is the Knowledge object with parse_status reset to the re-processing state; poll `fmind doc wait <doc-id>` for completion",
	})
	return cmd
}
...
func runReparse(ctx context.Context, opts *ReparseOptions, fopts *cmdutil.FormatOptions, svc ReparseService, id string) error {
	...
	fmt.Fprintf(w, "  next: fmind doc wait %s\n", id)
	return nil
}
