...
// NewCmdUpload builds `fmind doc upload <file>`.
...
directory tree (see Examples). To ingest a remote URL use "fmind doc fetch";
to create an entry from inline text use "fmind doc create".
...
Server-side ingestion knobs:
  ...
  --channel <name>         Override the ingestion-channel tag (default "api").
                           Applies to file / --recursive.`,
		Example: `  fmind doc upload report.pdf
  fmind doc upload notes.md --kb a32a63ff-fb36-4874-bcaa-30f48570a694
  fmind doc upload notes.md --kb my-kb
  fmind doc upload q3.pdf --name "Q3 Marketing Report.pdf"
  fmind doc upload report.pdf --enable-multimodel --metadata team=alpha --metadata sprint=Q4
  fmind doc upload ./docs --recursive --glob '*.pdf' --metadata team=alpha`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:       "Upload a local file to the resolved knowledge base. KB resolved via --kb flag, FMIND_KB_ID env, or project link. Emits the created Knowledge object with its id.",
		RequiredFlags: []string{"<file> (positional)", "--kb (or FMIND_KB_ID / project link)"},
		Examples: []string{
			`fmind doc upload report.pdf --kb kb_eng`,
			`fmind doc upload report.pdf --kb kb_eng --jq .data.id   # then: doc wait <id>`,
		},
...
func validateUploadFlags(opts *UploadOptions, args []string) error {
	...
	if !hasPath {
		...
		return cmdutil.NewFlagError(errors.New(
			"a file path is required (or use `fmind doc fetch` for URLs, `fmind doc create` for inline text)"))
	}
	return nil
}
