...
// NewCmdUpload builds `keystone doc upload <file>`.
...
directory tree (see Examples). To ingest a remote URL use "keystone doc fetch";
to create an entry from inline text use "keystone doc create".
...
Server-side ingestion knobs:
  ...
  --channel <name>         Override the ingestion-channel tag (default "api").
                           Applies to file / --recursive.`,
		Example: `  keystone doc upload report.pdf
  keystone doc upload notes.md --kb a32a63ff-fb36-4874-bcaa-30f48570a694
  keystone doc upload notes.md --kb my-kb
  keystone doc upload q3.pdf --name "Q3 Marketing Report.pdf"
  keystone doc upload report.pdf --enable-multimodel --metadata team=alpha --metadata sprint=Q4
  keystone doc upload ./docs --recursive --glob '*.pdf' --metadata team=alpha`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:       "Upload a local file to the resolved knowledge base. KB resolved via --kb flag, KEYSTONE_KB_ID env, or project link. Emits the created Knowledge object with its id.",
		RequiredFlags: []string{"<file> (positional)", "--kb (or KEYSTONE_KB_ID / project link)"},
		Examples: []string{
			`keystone doc upload report.pdf --kb kb_eng`,
			`keystone doc upload report.pdf --kb kb_eng --jq .data.id   # then: doc wait <id>`,
		},
...
func validateUploadFlags(opts *UploadOptions, args []string) error {
	...
	if !hasPath {
		...
		return cmdutil.NewFlagError(errors.New(
			"a file path is required (or use `keystone doc fetch` for URLs, `keystone doc create` for inline text)"))
	}
	return nil
}
