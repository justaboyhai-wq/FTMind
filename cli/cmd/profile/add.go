...
// NewCmdAdd builds `fmind profile add`. Registers a *credentialless*
...
// `fmind auth login`, separating "where" the CLI talks to (the host) and
// "how" it authenticates (the credential). Pass --use to switch to the new
// profile immediately, then run `fmind auth login`.
...
profile active (` + "`fmind profile use <n>`" + `, or ` + "`profile add <n> --use`" + `) and run
` + "`fmind auth login`" + `.
...
adds leave the current profile untouched unless --use is passed.`,
		Example: `  fmind profile add staging --host https://staging.example.com
  fmind profile add prod    --host https://prod.example.com --user alice@example.com
  fmind profile add prod    --host https://prod.example.com --use   # switch to it now`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if _, exists := cfg.Profiles[name]; exists {
				return &cmdutil.Error{
					...
					Hint:    fmt.Sprintf("use a different name, or run `fmind profile remove %s` first", name),
				}
			}
...
func runAddWithConfig(opts *AddOptions, fopts *cmdutil.FormatOptions, name, host string, cfg *config.Config) error {
	...
	if current {
		fmt.Fprintf(iostreams.IO.Out, "✓ Added profile %s (now current). Run `fmind auth login` to attach credentials.\n", name)
	} else {
		fmt.Fprintf(iostreams.IO.Out, "✓ Added profile %s. Run `fmind profile use %s` then `fmind auth login` to attach credentials.\n", name, name)
	}
	return nil
}
