...
import (
	...
	agentcmd "github.com/justaboyhai-wq/fmind/cli/cmd/agent"
	apicmd "github.com/justaboyhai-wq/fmind/cli/cmd/api"
	"github.com/justaboyhai-wq/fmind/cli/cmd/auth"
	chatcmd "github.com/justaboyhai-wq/fmind/cli/cmd/chat"
	chunkcmd "github.com/justaboyhai-wq/fmind/cli/cmd/chunk"
	configcmd "github.com/justaboyhai-wq/fmind/cli/cmd/config"
	"github.com/justaboyhai-wq/fmind/cli/cmd/doc"
	"github.com/justaboyhai-wq/fmind/cli/cmd/doctor"
	"github.com/justaboyhai-wq/fmind/cli/cmd/kb"
	linkcmd "github.com/justaboyhai-wq/fmind/cli/cmd/link"
	messagecmd "github.com/justaboyhai-wq/fmind/cli/cmd/message"
	mcpcmd "github.com/justaboyhai-wq/fmind/cli/cmd/mcp"
	modelcmd "github.com/justaboyhai-wq/fmind/cli/cmd/model"
	profilecmd "github.com/justaboyhai-wq/fmind/cli/cmd/profile"
	"github.com/justaboyhai-wq/fmind/cli/cmd/search"
	sessioncmd "github.com/justaboyhai-wq/fmind/cli/cmd/session"
	skillscmd "github.com/justaboyhai-wq/fmind/cli/cmd/skills"
...
a curated read-only MCP tool surface for AI agents.`,
  ...
  fmind doctor --format json`,
		...
		PersistentPreRunE: func(c *cobra.Command, args []string) error {
			...
			// Propagate the global --profile flag (or FMIND_PROFILE env) into
			...
			if v, _ := c.Flags().GetString("profile"); v != "" {
				f.ProfileOverride = v
			} else if v := os.Getenv("FMIND_PROFILE"); v != "" {
				f.ProfileOverride = v
			}
			...
			// Resolve --log-level / FMIND_LOG_LEVEL and apply to the SDK
	...
	// Match `fmind version` line format so both forms output the same.
	cmd.SetVersionTemplate("fmind {{.Version}}\n")
	addGlobalFlags(cmd)
...
func addGlobalFlags(cmd *cobra.Command) {
	...
	// (e.g. `fmind fooo --format json`) reach the typed-envelope guard
	...
	pf.String("format", "", "Output format: text | json | ndjson (default: json; env: FMIND_FORMAT)")
...
func newVersionCmd(f *cmdutil.Factory) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			fmt.Fprintf(c.OutOrStdout(), "fmind %s (commit %s, built %s)\n", v, commit, date)
			return nil
		},
	}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{"fmind version --format json"},
...
// (e.g. `fmind kb bogus`). Without this, cobra falls back to a free-form
...
func unknownSubcommandRunE(cmd *cobra.Command, args []string) error {
	// Group command invoked with no subcommand (e.g. `fmind kb`):
