...
func TestNewCmdChunks_NoKBUsesResolver(t *testing.T) {
	iostreams.SetForTest(t)
	t.Setenv("FMIND_KB_ID", "") // no ambient KB from env
	t.Chdir(t.TempDir())          // no .fmind project link discoverable
...
// FMIND_KB_ID set and no --kb, KB resolution succeeds (no kb-required
...
func TestNewCmdChunks_HonorsKBEnv(t *testing.T) {
	iostreams.SetForTest(t)
	t.Setenv("FMIND_KB_ID", "kb_from_env")
	cmd := NewCmdChunks(&cmdutil.Factory{
