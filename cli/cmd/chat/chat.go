// Package chat implements `fmind chat <text>` - the streaming RAG answer
// entry point.
//
...
both flags for the complete projected stream.`,
		Example: `  fmind chat "What is RRF?" --kb a32a63ff-fb36-4874-bcaa-30f48570a694
  fmind chat "Summarise this design doc" --kb my-kb --format json
  fmind chat "Continue?" --session sess_abc`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			`fmind chat "What is RRF?" --kb kb_abc`,
			`fmind chat "What is RRF?" --kb kb_abc --jq '[.data.events[].content] | join("")'`,
		},
...
func runChat(ctx context.Context, opts *Options, fopts *cmdutil.FormatOptions, svc ChatService) error {
	...
	autoCreated := false
	if sessionID == "" {
		sess, err := svc.CreateSession(ctx, &sdk.CreateSessionRequest{Title: "fmind chat"})
		if err != nil {
