// Package skillscmd holds the `fmind skills` command tree: list the bundled
...
import (
	...
	skillsfs "github.com/justaboyhai-wq/fmind/cli/skills"
)

// NewCmd builds the `fmind skills` parent command.
...
func newCmdList() *cobra.Command {
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:  "list the Agent Skills bundled in this CLI binary (name, description, file count) — discover what `fmind skills install` would write",
		Examples: []string{"fmind skills list --format json"},
...
Existing files are left untouched unless --force is passed.`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{"fmind skills install --dry-run --format json", "fmind skills install --dir ~/.claude/skills --force"},
