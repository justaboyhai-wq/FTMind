...
// can re-run `fmind auth status` (live API check).
type refreshResult struct {
	Profile string `json:"profile"`
}

// NewCmdRefresh builds `fmind auth refresh`. Renews the JWT access
...
func NewCmdRefresh(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Reads the refresh token previously stored by ` + "`fmind auth login`" + ` and
...
refresh semantic. Rotate the key in the server UI instead.`,
		Example: `  fmind auth refresh                      # refresh the active profile
  fmind --profile staging auth refresh   # refresh a specific profile`,
		Args: cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
			...
			if name == "" {
				...
				return cmdutil.NewError(cmdutil.CodeAuthUnauthenticated,
					"no active profile configured; run `fmind auth login` to set one up")
			}
			...
			if prof.RefreshRef == "" {
				hint := "api-key profiles can't be refreshed - rotate the key in the server UI and run `fmind auth login --with-token`"
				if prof.APIKeyRef == "" {
					hint = "no refresh token stored - run `fmind auth login` to authenticate"
				}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"fmind auth refresh",
			"fmind --profile staging auth refresh",
		},
	})
	return cmd
}
