...
// NewCmdUnlink builds `keystone unlink`. Walks up from cwd so a user in
...
func NewCmdUnlink() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Removes the .keystone/project.yaml that ` + "`keystone link`" + ` previously
...
is present anywhere in the parent chain.`,
		Example: `  keystone unlink           # remove the binding for this project
  keystone unlink --format json    # bare JSON (CI / agents)`,
		Args: cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
			...
			if !found {
				return &cmdutil.Error{
					...
					Hint:    "run `keystone link --kb <id>` first, or check you're in the right directory",
				}
			}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor: "Remove the .keystone/project.yaml KB binding from the current directory tree. No flags required; walks up from cwd to find the link.",
