// Package linkcmd implements `fmind link` - binds the current working
// directory to a knowledge base by writing .fmind/project.yaml. Always
...
// NewCmd builds the `fmind link` command.
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	opts := &Options{}
	cmd := &cobra.Command{
		Use:   "link [kb]",
		Short: "Bind the current directory to a knowledge base",
		Long: `Writes .fmind/project.yaml in the current working directory pointing
...
overridden by the --kb flag or FMIND_KB_ID env var.

Pass the knowledge base as a positional argument (fmind link <id-or-name>) or
...
user explicitly asked to bind this directory; don't run it as a side effect.`,
		Example: `  fmind link a32a63ff-fb36-4874-bcaa-30f48570a694         # positional UUID
  fmind link engineering                                  # positional name → id
  fmind link --kb engineering                             # --kb form (equivalent)
  fmind link                                              # interactive (TTY)`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:       "Bind the current directory to a knowledge base by writing .fmind/project.yaml. Give the KB as a positional arg or --kb (non-interactive); only run when the user explicitly asks to link this directory.",
		RequiredFlags: []string{"<kb> positional or --kb (required when no TTY)"},
		Examples:      []string{"fmind link engineering", "fmind link --kb a32a63ff-fb36-4874-bcaa-30f48570a694"},
...
// per-invocation override flag on `fmind link` itself - to record under a
// different profile, use the global persistent flag (`fmind --profile
...
func resolveProfile(f *cmdutil.Factory) (string, error) {
	...
	if cfg.CurrentProfile == "" {
		...
		// profile — env credentials (FMIND_API_KEY) alone have no profile to
		...
		return "", cmdutil.NewError(cmdutil.CodeAuthUnauthenticated,
			...
			WithHint("register one with `fmind profile add <name> --host <url> --use`; for a headless (FMIND_API_KEY) workflow, skip `link` and pass --kb per command or set FMIND_KB_ID").
