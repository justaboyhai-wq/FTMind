...
const chunkDeleteExample = `  fmind chunk delete chunk_abc --doc doc_xyz                  # interactive confirm
  fmind chunk delete chunk_abc --doc doc_xyz -y               # no prompt
  fmind chunk delete chunk_abc --doc doc_xyz -y --format json # bare {id, deleted:true} JSON
  fmind chunk delete c1 c2 c3 --doc doc_xyz -y                # delete 3 chunks under same doc, keep-going`

// NewCmdDelete builds `fmind chunk delete <chunk-id> [<chunk-id>...] --doc <doc-id>`.
func NewCmdDelete(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			batchRetry := append([]string{"fmind", "chunk", "delete"}, args...)
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"fmind chunk delete chunk_abc --doc doc_xyz -y",
			"fmind chunk delete c1 c2 c3 --doc doc_xyz -y",
			"fmind chunk delete chunk_abc --doc doc_xyz -y --format json",
		},
...
func runDelete(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc DeleteService, p prompt.Prompter) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "chunk", opts.ChunkID, "chunk.delete", []string{"fmind", "chunk", "delete", opts.ChunkID, "--doc", opts.DocID, "-y"}); err != nil {
		return err
	}
