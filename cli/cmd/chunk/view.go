...
less or use --format json for large chunks. Keystone chunks are typically bounded
...
const chunkViewExample = `  keystone chunk view chunk_abc
  keystone chunk view chunk_abc --format json | jq '.content'
  keystone chunk view chunk_abc --format json --jq '{id, chunk_index, is_enabled}'`

// NewCmdView builds `keystone chunk view <chunk-id>`.
func NewCmdView(f *cmdutil.Factory) *cobra.Command {
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples:      []string{"keystone chunk view chunk_abc"},
