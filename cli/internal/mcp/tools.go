...
func addChat(server *mcpsdk.Server, svc chatService) {
	mcpsdk.AddTool(server, &mcpsdk.Tool{
		...
		if sessionID == "" {
			sess, err := svc.CreateSession(ctx, &sdk.CreateSessionRequest{Title: "fmind mcp chat"})
			if err != nil {
...
func addSessionAsk(server *mcpsdk.Server, svc sessionAskService) {
	mcpsdk.AddTool(server, &mcpsdk.Tool{
		...
		sessionID := in.SessionID
		if sessionID == "" {
			sess, err := svc.CreateSession(ctx, &sdk.CreateSessionRequest{Title: "fmind mcp session_ask"})
			if err != nil {
				return toolErrorResult(cmdutil.WrapHTTP(err, "create chat session")), nil, nil
			}
			sessionID = sess.ID
