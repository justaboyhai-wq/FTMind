...
// names the bundled `keystone-shared` skill advertises (in its "CLI vs MCP"
...
func TestSkillReferencedMCPToolsExist(t *testing.T) {
	...
	skillPath := filepath.Join(filepath.Dir(file), "..", "..", "skills", "keystone-shared", "SKILL.md")
	...
	if start < 0 {
		t.Fatal("keystone-shared SKILL.md has no 'CLI vs MCP' section to validate")
	}
	section := body[start:]
	...
	for name := range referenced {
		if !registered[name] {
			t.Errorf("keystone-shared skill references MCP tool %q which is NOT registered (renamed/removed?)", name)
		}
	}
}
