...
// DefaultFormatMode is the mode used when neither --format nor FMIND_FORMAT
...
func (o *FormatOptions) ResolveDefault(tty bool) {
	o.TTY = tty
	// Apply FMIND_FORMAT before the hard default so the documented
	// precedence holds: explicit --format (already set on o.Mode by
	// CheckFormatFlag) > FMIND_FORMAT > DefaultFormatMode. FromEnv is a
...
// FromEnv reads FMIND_FORMAT and applies it when Mode hasn't been set
