...
// profile is configured (the precondition for `fmind auth login`).
...
func TestFactory_ProfileOverride(t *testing.T) {
	...
	cfgPath := dir + "/fmind/config.yaml"
	require.NoError(t, os.MkdirAll(dir+"/fmind", 0o700))
...
func TestBuildClient_NoCurrentProfile(t *testing.T) {
	...
	assert.NotEqual(t, []string{"fmind", "auth", "login"}, detail.RetryArgv,
...
// TestFactory_ActiveProfile_EnvVarFallback verifies FMIND_PROFILE is honoured
// when no override or config is present.
func TestFactory_ActiveProfile_EnvVarFallback(t *testing.T) {
	t.Setenv("FMIND_PROFILE", "staging")
...
// priority over the FMIND_PROFILE env var.
...
func TestResolveKB_Chain(t *testing.T) {
	t.Run("flag_kb_id_wins", func(t *testing.T) {
		...
		t.Setenv("FMIND_KB_ID", "kb_env_should_lose")
		dir := t.TempDir()
		resolveKBChdir(t, dir)
		// Drop a project link too - must be ignored.
		require.NoError(t, projectlink.Save(filepath.Join(dir, ".fmind", "project.yaml"), &projectlink.Project{KBID: "kb_disk_should_lose"}))
	...
	t.Run("project_link_walk_up", func(t *testing.T) {
		t.Setenv("FMIND_KB_ID", "")
		root := t.TempDir()
		require.NoError(t, projectlink.Save(filepath.Join(root, ".fmind", "project.yaml"), &projectlink.Project{KBID: "kb_proj"}))
...
// TestBuildClientFromEnv covers the stateless env-credential path: FMIND_TOKEN
// / FMIND_API_KEY build an ephemeral client with no config/keyring access.
func TestBuildClientFromEnv(t *testing.T) {
	...
	t.Run("token set but no host is a typed input error", func(t *testing.T) {
		t.Setenv("FMIND_API_KEY", "")
		t.Setenv("FMIND_TOKEN", "jwt-token")
		t.Setenv("FMIND_HOST", "")
		c, handled, err := buildClientFromEnv(emptyCfg)
	...
	t.Run("host falls back to the active profile when FMIND_HOST unset", func(t *testing.T) {
		t.Setenv("FMIND_API_KEY", "")
		t.Setenv("FMIND_TOKEN", "jwt-token")
		t.Setenv("FMIND_HOST", "")
