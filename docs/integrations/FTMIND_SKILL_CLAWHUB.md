# FTMind Skill / ClawHub 发布说明

当前 Skill 包位于 `frontend/public/ftmind-skill`，版本为 `2.0.0`，入口文件是
`SKILL.md`。它同时覆盖普通知识库 API 和 FTMind/MemoryProxy 外部记忆网关，
...
脚本不会读取或写入 FTMind 密钥，也不会自动登录或从浏览器推导地址。若当前
