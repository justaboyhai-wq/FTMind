...
启动方式：`docker compose --profile doris up -d`，然后在 FE 上 `CREATE DATABASE keystone;`。
...
`TENCENT_VECTORDB_COLLECTION` 是集合名前缀。Keystone 会按向量维度创建实际集合，例如 `keystone_embeddings_768`。
