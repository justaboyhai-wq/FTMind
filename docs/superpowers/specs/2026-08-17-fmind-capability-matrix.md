# FTMind 与 PRD MVP 能力矩阵

状态含义：

- `reuse`：FTMind 已有实现，先做契约和回归验收；
...
|---|---|---|---|
| ORG | FR-ORG-001–005, FR-ORG-008 | reuse/adapt | FTMind Tenant、Organization、成员、角色、上下文；Task 0、Task 1、Task 2 |
| ORG | FR-ORG-006 | adapt | FTMind 服务身份/API Key；增加 Connector Secret 与短期 Binding Token；Task 2、Task 3 |
...
| AGT | FR-AGT-004 | reuse/adapt | FTMind MCP 服务、工具注册和审批；Cognition MCP 增加外部记忆工具；Task 13 |
...
| MEM | FR-MEM-003 | adapt | MemoryCore L1 提取；FTMind 增加纠正/确认/撤回；Task 8 |
| MEM | FR-MEM-004 | adapt | MemoryCore L2 聚合；FTMind 增加范围、证据和结果确认；Task 8 |
| MEM | FR-MEM-005 | reuse/adapt | MemoryCore L3 沉淀；FTMind 接收成熟事件；Task 7、Task 10 |
...
| RAW | FR-RAW-001–012 | reuse/adapt | FTMind Knowledge/Storage/Source/ACL/Lineage；本轮记忆 Wiki 不进入 Raw；Task 0 矩阵和现有回归 |
...
| SKL | FR-SKL-001–008 | reuse/defer | FTMind Skill 读取/执行基础保留；完整 Skill 资产、提炼和跨 Agent 分发另立计划 |
...
| 输入 | IN-02 文件上传 | FTMind 现有上传回归，不与记忆 Wiki 混用；Task 0 |
| 输入 | IN-03 外部数据源 | FTMind 现有 Connector 回归；Task 0 |
| 输入 | IN-04 URL/结构化数据 | FTMind 现有导入回归；Task 0 |
| 输入 | IN-05 Agent 交付物 | FTMind 现有 Raw/知识入口回归；Task 0 |
...
| 输出 | OUT-02 RAG 问答 | FTMind 既有知识库回归；不与 MemoryCore 召回合并；Task 0 |
| 输出 | OUT-03 Wiki 导航 | FTMind Wiki 回归和记忆 Wiki 标识；Task 11、14 |
| 输出 | OUT-04 Raw 原件访问 | FTMind 既有原件权限回归；Task 0 |
...
- MemoryCore 的 L0/L1 向量召回和 L2/L3 Profile write-through 必须保留，
  但与 FTMind RAG collection、生命周期、权限和索引完全隔离；
- FTMind 原有记忆开关和 Neo4j 路径回归不受影响；
