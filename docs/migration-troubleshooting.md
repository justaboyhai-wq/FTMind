...
This guide is linked from the system info page when Keystone's startup database
...
Keystone auto-runs `golang-migrate` migrations on every startup. When a
...
Then restart Keystone. The next startup will pick up where the failing
migration left off.
...
default, Keystone's startup tries to auto-recover; if you disabled that with
...
4. **Fix the underlying cause** (install extension, fix privileges, free
   disk, …), then either:
   - Restart Keystone and let auto-recovery retry; **or**
...
<https://github.com/justaboyhai-wq/keystone/issues/new?template=bug_report.yml>

Include:

- Keystone version + commit ID (from the system info page).
