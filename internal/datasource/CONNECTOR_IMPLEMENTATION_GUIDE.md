...
A connector is an adapter that translates between Keystone's data model and an external platform's API. It handles:
...
func (c *YourConnector) ListResources(ctx context.Context, config *types.DataSourceConfig) ([]types.Resource, error) {
    ...
    // Convert to Keystone Resource format
...
import (
    // ... existing imports
    yourconnector "github.com/justaboyhai-wq/keystone/internal/datasource/connector/yourtype"
)
...
2. Keystone already has `internal/im/feishu/` for reference
