...
A connector is an adapter that translates between FTMind's data model and an external platform's API. It handles:
...
func (c *YourConnector) ListResources(ctx context.Context, config *types.DataSourceConfig) ([]types.Resource, error) {
    ...
    // Convert to FTMind Resource format
...
import (
    // ... existing imports
    yourconnector "github.com/justaboyhai-wq/fmind/internal/datasource/connector/yourtype"
)
...
2. FTMind already has `internal/im/feishu/` for reference
