...
import (
	...
	_ "github.com/justaboyhai-wq/fmind/docs" // swagger docs
)
...
func NewRouter(params RouterParams) *gin.Engine {
	...
	// FMIND_TRUSTED_PROXIES (comma-separated CIDRs/IPs).
...
// with FMIND_TRUSTED_PROXIES (comma-separated). An explicit empty value
// disables proxy trust entirely so ClientIP() returns the direct peer.
func trustedProxies() []string {
	raw, ok := os.LookupEnv("FMIND_TRUSTED_PROXIES")
	if !ok {
		return []string{
			"127.0.0.0/8",
			"::1/128",
...
func serveFrontendStatic(r *gin.Engine) {
	webDir := os.Getenv("FMIND_WEB_DIR")
	if webDir == "" {
		webDir = "./web"
	}
