export default {
  ...
  settings: {
    ...
    storage: {
      ...
      configurable: "可配置",
      pathPrefix: "路径前缀（可选）",
      pathPrefixPlaceholder: "如 fmind/images",
      prefixPlaceholder: "如 fmind",
  ...
  system: {
    ...
    retry: "重试",
    versionLabel: "应用版本",
    versionDescription: "当前应用服务（fmind-app）的版本号",
    frontendVersionLabel: "UI 版本",
    frontendVersionDescription: "当前 UI 界面（fmind-ui）的构建版本号",
  ...
  integrations: {
    ...
    api: {
      ...
      signedRequestStep0:
        "# 0. 在你的后端签发 JWT（HS256，aud=fmind，sub=用户 ID，tenant_id={tenantId}，exp 不超过 24 小时）",
  ...
  externalMemory: {
    eyebrow: '记忆治理',
    title: '外部记忆',
    subtitle: '管理外部智能体接入与已审核的 L3 记忆，不改变 FTMind 原有知识检索路径。',
    refresh: '刷新',
    scopeNote: '仅当前空间管理员可用。接入密钥仅在创建或轮换时展示一次。',
    memoryWikiBadge: '记忆知识库',
    tabs: { bindings: '智能体接入', reviews: 'L3 审核与发布' },
    overview: {
      title: '外部记忆概览', desc: '查看外部智能体接入与 L3 记忆审核状态。',
      activeBindings: '已启用接入', activeBindingsHint: '当前可用的外部智能体接入',
      revokedBindings: '已撤销接入', revokedBindingsHint: '已停止访问的外部智能体接入',
      pendingReviews: '待审核记忆', pendingReviewsHint: '等待审核或需要修改的 L3 记忆',
      publishedReviews: '已发布记忆', publishedReviewsHint: '已发布至记忆知识库的 L3 记忆',
      openBindings: '管理接入', openReviews: '查看审核', wikiHint: '只有审核通过的 L3 记忆才能发布到专用记忆知识库。',
    },
    bindings: {
      title: '智能体接入', desc: '每个绑定仅允许一个外部智能体在指定团队及明确能力范围内使用记忆。',
      create: '创建绑定', createTitle: '创建外部智能体绑定', empty: '还没有外部智能体绑定。',
      rotate: '轮换密钥', revoke: '撤销', revokeTitle: '撤销绑定？',
      revokeHint: '撤销后，{name} 将立即被禁止获取新的记忆访问令牌。', revoked: '绑定已撤销', rotated: '接入密钥已轮换',
    },
    fields: {
      department: '所属部门', teamId: '团队编号', userId: '用户编号', agentId: 'FTMind 智能体编号', externalAgent: '外部智能体编号',
      connector: '接入类型', capabilities: '能力范围', assetScopes: '资源范围', assetScopesHelp: '每行一个或以逗号分隔，例如 team:engineering。',
      selectDepartment: '请选择部门', selectTeam: '请选择团队', selectAgent: '请选择智能体', selectUser: '请选择用户',
      selectOrganizationFirst: '请先选择部门、团队、智能体和用户。', noTeams: '当前部门暂无团队。', organizationLoadFailed: '加载组织信息失败。',
      userApiKey: '用户访问密钥', userApiKeyHelp: '选择绑定用户已有的 FTMind 访问密钥；提示词仅使用环境变量占位符。',
      selectUserApiKey: '请选择用户访问密钥', unassignedKey: '未绑定用户',
    },
    connectorTypes: { openclaw_plugin: 'OpenClaw 插件', hermes_provider: 'Hermes 服务提供方', openai_proxy: 'OpenAI 代理', anthropic_proxy: 'Anthropic 代理', mcp: 'MCP 协议', generic_sdk: '通用开发工具包' },
    capabilities: { memory_context: '使用记忆上下文', memory_capture: '采集记忆', memory_recall: '召回记忆', memory_confirm: '确认记忆候选项', memory_publish: '发布记忆知识库', knowledge_search: '检索知识库', wiki_get: '读取知识库页面', document_read: '读取文档', context_assemble: '组织上下文' },
    policy: { capture: '采集记忆', recall: '召回记忆', l3: '启用 L3 记忆知识库发布' },
    secret: { title: '保存接入密钥', warning: '请立即复制密钥，关闭此对话框后将不能再次查看。', copy: '复制', copied: '密钥已复制', copyFailed: '复制失败，请手动选中密钥复制。', confirm: '已安全保存' },
    setupWizard: { title: '完成外部智能体配置', warning: '请妥善保管以下配置内容，关闭窗口后将不能再次查看。', expires: '有效期至：', instructions: '将以下配置提示词提供给外部智能体完成接入。', copyPrompt: '复制配置提示词', copied: '配置提示词已复制' },
    reviews: { title: 'L3 审核与发布', desc: '只有审核通过的 L3 记忆才能发布到专用记忆知识库。', allStatuses: '所有状态', empty: '暂无 L3 记忆待处理。', open: '审核', detailTitle: 'L3 记忆审核', markdown: '正文', evidence: '证据', noEvidence: '当前没有记录证据。', reviewComment: '审核评论', comment: '评论', commentRequired: '要求修改时必须填写说明。', commentOptional: '可选评论' },
    actions: { approve: '通过', reject: '拒绝', requestChanges: '要求修改', publish: '发布到记忆知识库', saved: '审核已保存', published: '已发布到记忆知识库' },
    columns: { externalAgent: '外部智能体', connector: '接入', team: '团队', policy: '策略', status: '状态', actions: '操作', title: 'L3 标题', agent: '智能体' },
    status: { pending_setup: '待完成配置', pending_review: '待审核', changes_requested: '需修改', approved: '已通过', publishing: '正在发布', published: '已发布', rejected: '已拒绝', revoked: '已撤销', active: '已启用' },
    detail: { memoryId: '记忆编号' },
    bindingError: '智能体绑定参数无效，请检查后重试。',
    validation: { required: '该字段为必填项。' }, error: { load: '加载外部记忆数据失败。', save: '保存失败，请稍后再试。' },
  },
};
