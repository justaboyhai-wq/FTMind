...
const protectedFileCacheState = (() => {
  ...
  const scope = window as typeof window & {
    __keystoneProtectedFileCacheV1__?: ProtectedFileCacheState;
  };
  scope.__keystoneProtectedFileCacheV1__ ||= fresh();
  return scope.__keystoneProtectedFileCacheV1__;
})();
...
function getProtectedFileRequestHeaders(): Record<string, string> {
  ...
  try {
    const token = (localStorage.getItem('keystone_token') || '').trim();
    if (token) {
      headers['Authorization'] = `Bearer ${token}`;
    }

    const selectedTenantId = (localStorage.getItem('keystone_selected_tenant_id') || '').trim();
    if (selectedTenantId) {
      ...
      // active tenant into keystone_tenant, leaving authenticated file
