...
const CHAT_FILE_DROP_EVENT = 'keystone:chat-file-drop';
...
const LAST_CHAT_MODEL_KEY = 'keystone_last_chat_model_id'
        ...
        <span v-for="item in allSelectedItems" :key="`${item.type}:${item.id}`" class="mention-chip" :class="[
          ...
          <span class="mention-chip__icon-wrap" :class="{ 'has-org': item.org_name }">
            ...
            <span v-if="item.org_name" class="mention-chip__org-badge">
              <t-icon name="usergroup" class="mention-chip__org-img"
                :class="{ 'mention-chip__org-img--muted': item.type === 'file' }" aria-hidden="true" />
            </span>
          </span>
...
.rich-input-container {
  ...
  &:focus-within {
    border-color: var(--td-brand-color, #5b5bd6);
  }
}
...
.mention-chip--faq .mention-chip__icon-wrap {
  color: var(--keystone-faq-color, #0052d9);
}
...
.image-upload-btn {
  ...
  .image-count {
    ...
    top: -2px;
    right: -2px;
    background: var(--td-brand-color, #5b5bd6);
    color: #fff;
    font-size: 10px;
    width: 14px;
    height: 14px;
