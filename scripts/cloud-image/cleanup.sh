...
echo "[cleanup] 1/8 停止 Keystone 容器"
COMPOSE_PROJECT=""
if [[ -d "${KEYSTONE_DIR}" ]]; then
  cd "${KEYSTONE_DIR}"
  # 优先用 compose ls 拿到真实 project 名 (默认是目录名小写, 如 keystone)
  COMPOSE_PROJECT="$(docker compose ls --format json 2>/dev/null \
    | grep -oE '"Name":"[^"]+"' | head -1 | cut -d'"' -f4 || true)"
  docker compose down -v --remove-orphans || true
fi

echo "[cleanup] 2/8 清空 Keystone 业务数据 + 首启 marker / 日志"
if [[ -d "${KEYSTONE_DIR}" ]]; then
  rm -rf "${KEYSTONE_DIR}/data"/* "${KEYSTONE_DIR}/logs"/* 2>/dev/null || true
  ...
  rm -f "${KEYSTONE_DIR}/.env" "${KEYSTONE_DIR}/.firstboot.done"
fi
rm -f /root/keystone-credentials.txt /var/log/keystone-firstboot.log
...
# wechatopenai/keystone-* 等镜像一并清掉, 导致基于镜像建出来的新实例
