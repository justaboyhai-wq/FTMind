...
if [[ -d "${FMIND_DIR}" ]]; then
  cd "${FMIND_DIR}"
  # 优先用 compose ls 拿到真实 project 名 (默认是目录名小写, 如 fmind)
...
if [[ -d "${FMIND_DIR}" ]]; then
  ...
  rm -f "${FMIND_DIR}/.env" "${FMIND_DIR}/.firstboot.done"
fi
rm -f /root/fmind-credentials.txt /var/log/fmind-firstboot.log
...
# wechatopenai/fmind-* 等镜像一并清掉, 导致基于镜像建出来的新实例
