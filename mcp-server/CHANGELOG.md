...
4. `python keystone_mcp_server.py` - 直接运行
5. `python -m keystone_mcp_server` - 模块运行
6. `keystone-mcp-server` - 安装后命令行工具
7. `keystone-server` - 安装后命令行工具 (别名)
