...
func checkUpdate(ctx context.Context, currentVersion string, showUpToDate bool, autoDownload bool) {
	go func() {
		...
		req, err := http.NewRequest("GET", "https://api.github.com/repos/justaboyhai-wq/keystone/releases/latest", nil)
		if err != nil {
		...
		req.Header.Set("User-Agent", "Keystone-Lite-Desktop-App")
		...
		if showUpToDate {
			wailsruntime.MessageDialog(ctx, wailsruntime.MessageDialogOptions{
				Type:          wailsruntime.InfoDialog,
				Title:         "Up to Date",
				Message:       fmt.Sprintf("You are using the latest version of Keystone Lite.\n\nCurrent version: %s", currentVersion),
				Buttons:       []string{"OK"},
				DefaultButton: "OK",
			})
		}
...
func applyUpdateAndRestart(ctx context.Context, savePath string) {
	if runtime.GOOS == "windows" {
		scriptPath := filepath.Join(os.TempDir(), "keystone_update.bat")
...
`, savePath, execPath)
		...
		if strings.HasSuffix(strings.ToLower(savePath), ".dmg") {
			go func() {
				...
				mountPoint := filepath.Join(os.TempDir(), "KeystoneUpdateMount")
				...
				scriptPath := filepath.Join(os.TempDir(), "keystone_update.sh")
