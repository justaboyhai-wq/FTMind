...
func checkUpdate(ctx context.Context, currentVersion string, showUpToDate bool, autoDownload bool) {
	go func() {
		...
		req, err := http.NewRequest("GET", "https://api.github.com/repos/justaboyhai-wq/fmind/releases/latest", nil)
		if err != nil {
...
func applyUpdateAndRestart(ctx context.Context, savePath string) {
	if runtime.GOOS == "windows" {
		scriptPath := filepath.Join(os.TempDir(), "fmind_update.bat")
...
`, savePath, execPath)
		...
		if strings.HasSuffix(strings.ToLower(savePath), ".dmg") {
			go func() {
				...
				scriptPath := filepath.Join(os.TempDir(), "fmind_update.sh")
