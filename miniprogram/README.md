...
This directory contains a WeChat Mini Program plugin for FMind. It gives mobile users a lightweight entry point to:

- configure a FMind API endpoint and tenant API key;
- list available knowledge bases;
- import a URL into a selected knowledge base;
- ask a selected knowledge base through FMind knowledge chat.
...
3. Open the **Settings** tab and fill in:
   - API Base URL, for example `https://fmind.example.com`;
   - API Key from the FMind tenant settings page.
...
- WeChat DevTools may block `localhost` requests when URL validation is enabled. For local testing, either disable domain validation in DevTools or expose FMind through a HTTPS development domain.
- In production Mini Programs, add the FMind API domain to the Mini Program request domain allowlist.
